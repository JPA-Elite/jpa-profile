...
async function loadMusic(page = 1) {
    ...
    try {
        ...
        <tr>
            ...
            <td>
                <img src="${music.image}" onclick="window.location.href='${music.image}'" alt="${
                music.title}" style="width: 60px; height: auto; border-radius:4px;">
            </td>
            <td>
...
async function updateMusic(musicId, formId) {
    ...
    const artist = form
        ...
        .value.trim();
    const musicFile = form.querySelector(`#${formId} input[id^='editMusicFile']`)
        .files[0];
...
async function deleteMusic(musicId, modalId) {
    const res = await fetch(`/admin/api/delete-music/${musicId}`, {
        method: "DELETE",
    });
